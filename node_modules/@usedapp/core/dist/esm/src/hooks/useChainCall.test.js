...
describe('useChainCall', () => {
    ...
    it('initial test balance to be correct', async () => {
        ...
        const { result, waitForCurrent } = await renderDAppHook(() => useChainCall(encodeCallData(callData, network1.chainId)), {
            config,
        });
    ...
    const testMultiChainUseChainCall = async (contract, args, chainId, endValue) => {
        const { result, waitForCurrent } = await renderDAppHook(() => useChainCall(encodeCallData({
            contract,
            method: 'balanceOf',
            args,
        }, chainId)), {
